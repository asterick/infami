...
#include "JalecoEarly0.h"

void JalecoEarly0::Initalize( int Mirroring )
{	
...
unsigned char JalecoEarly0::Read( unsigned short addr )
{
	switch( addr & 0xE000 )
	{
	case 0x0000:
...
void JalecoEarly0::Write( unsigned short addr, unsigned char ch )
{
	m_BusTrash = ch;
	switch( addr & 0xE000 )
	{
...
void JalecoEarly0::VideoAddressLatch( unsigned short Address )
{
	m_VideoAddress = Address;
}

void JalecoEarly0::VideoWrite( unsigned char Byte )
{
...
unsigned char JalecoEarly0::VideoRead() 
{ 
	switch( m_VideoAddress & 0x3000 )
	{
	case 0x0000:	// CHR-ROM
