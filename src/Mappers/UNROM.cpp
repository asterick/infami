...
void UNROM::Initalize( int Mirroring )
{	
...
unsigned char UNROM::Read( unsigned short addr )
{
	switch( addr & 0xE000 )
	{
	case 0x0000:
...
void UNROM::Write( unsigned short addr, unsigned char ch )
{
	m_BusTrash = ch;
	switch( addr & 0xE000 )
	{
	...
	case 0xC000:
	case 0xE000:
		m_Page = ((ch << 14) & m_PageMask) + PRGRom;
		break ;
	}
}